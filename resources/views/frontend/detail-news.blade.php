...
@section('meta-seo')
    ...
    <meta name="description" content="{{ $blog->meta_description != '' ? $blog->meta_description : \Str::limit($blog->excerpt, 160) }}" />
    <meta name="keywords"
    ...
    <meta property="og:description" content="{{ $blog->meta_description != '' ? $blog->meta_description : \Str::limit($blog->excerpt, 160) }}" />
    <meta property="og:image"
        content="{{ file_exists('uploads/' . $blog->featured_img) ? asset('uploads/' . $blog->featured_img) : asset('assets/img/default-blog-landscape.webp') }}" />
    <!-- Twitter -->
    ...
    <meta property="twitter:description" content="{{ $blog->meta_description != '' ? $blog->meta_description : \Str::limit($blog->excerpt, 160) }}" />
                    ...
                    <div class="blog-details__content">
                        <div class="blog-details__img-box1">
                            @if (file_exists('uploads/' . $blog->featured_img))
                                <div class="blog-details__img1">
                                    <img src="{{ file_exists('uploads/' . $blog->featured_img) ? asset('uploads/' . $blog->featured_img) : asset('assets/img/default-blog-potrait.png') }}"
                                        alt="image">
                                </div>
                            @endif
                                ...
                                <div class="meta-box">
                                    <ul class="meta-info">
                                        ...
                                        <li>
                                            ...
                                            <p><a href="#">{{ \Carbon\Carbon::parse($blog->created_at)->format('d F Y') }}</a></p>
                                        </li>
                                    </ul>
                                </div>
                                <div class="title-box">
                        ...
                        <div class="blog-details__tag-share">
                            <div class="tag2">
                                ...
                                <ul class="tag2-list">
                                    <li>
                                        <a href="#">{{ $blog->category->name ? Str::title($blog->category->name) : 'Artikel' }}</a>
                                    </li>
                                </ul>
                            </div>
                        ...
                        <div class="blog-details__prev-next-option">
                            <div class="single-box left">
                                <div class="icon">
                                    <a href="{{ $previous ? route('news.detail', $previous->slug) : '#' }}"><span class="icon-left-arrow"></span></a>
                                </div>
                                <div class="text">
                                    <p>Prev Blog</p>
                                    <h3><a href="{{ $previous ? route('news.detail', $previous->slug) : '#' }}">{{ $previous ? $previous->title : '' }}</a></h3>
                                </div>
                            </div>
                            <div class="single-box right">
                                <div class="text">
                                    <p>Next Blog</p>
                                    <h3><a href="{{ $next ? route('news.detail', $next->slug) : '#' }}">{{ $next ? $next->title : '' }}</a>
                                    </h3>
                                </div>
                                <div class="icon">
                                    <a href="{{ $next ? route('news.detail', $next->slug) : '#' }}"><span class="icon-right-arrow-angle"></span></a>
                                </div>
                            </div>
                        </div>
                    </div>
